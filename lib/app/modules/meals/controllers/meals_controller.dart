...
class MealsController extends GetxController {
  ...
  final _filterRepository = Get.find<FilterRepository>();

  List<Meal> get _categoryMealsList => _mealsList
      .where((meal) => meal.categories
          .contains(_filterRepository.selectedCategory.value!.id))
      .toList();

  List<Meal> get filteredMealsList => _categoryMealsList.where((meal) {
        if (_filterRepository.isGlutenFree.value && !meal.isGlutenFree) {
          return false;
        }
        if (_filterRepository.isLactoseFree.value && !meal.isLactoseFree) {
          return false;
        }
        if (_filterRepository.isVegetarian.value && !meal.isVegetarian) {
          return false;
        }
        if (_filterRepository.isVegan.value && !meal.isVegan) {
          return false;
        }
        return true;
      }).toList();
